...
## Current phase: F — prime finance / Stock Loan (in progress)

Phases A + C + D complete; **E core slice** merged (Developer Agent incident mode — `platform` MCP server, `investigate_incident`, revert/fix-forward hard rule, `POST /diagnose`, Engineering tab; verification / review / eval-authoring modes deferred → `docs/backlog.md`). **Phase F core slice is in progress** — the first prime-finance domain, **Stock Loan**: a `stockloan` MCP server (in-process, fixture-backed — loans, recalls, rerates, lending availability) with reads + three approval-gated writes (`initiate_recall`, `rerate_loan`, `book_buy_in`). `agent_core/stockloan.py` `investigate_loan(loan_id)` runs the `stockloan` specialist (scope `stockloan` + `market` + read-only `position` + `ops`); **recall vs buy-in is a hard rule in code** (`_enforce_recall_window`) keyed on `RECALL_NOTICE_DAYS` before `return_needed_by`. `POST /investigate {loan_id}`; the Supervisor can now decompose a `stockloan` sub-task (loan subject). Read `ai-platform/mcp_servers/stockloan/CLAUDE.md`. Deferred (`docs/backlog.md`): a seeded table + `simulator` planter (Python fixtures for now), the Margin / CorpActions / Cash domains, the mixed-client Scenario 30, a portal affordance. **No eval sweeps for C or later phases** (owner decision). When Phase F's core slice is merged, bump to "F — Stock Loan slice complete · next mainline phase: G".
