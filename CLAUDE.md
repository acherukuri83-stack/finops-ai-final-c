...
## Current phase: mainline A–G complete + the optional Wires module (complete)

Every mainline phase A–G has a merged core slice **and** its deferred depth — shipped, or closed-as-accepted with a rationale in `docs/backlog.md` (search `Closed (2026-09-09)`). The optional **Wires** module is **complete**: Wire specialist + `wire` MCP server + maker/checker/cutoff/screening hard rules, seeded Postgres + a `simulator` planter, Sc. 7 / 13–16 (`subject: wire`) ready for a sweep, `supervisor._held_wires` discovery, a wire corpus (`Wire Processing Guide`, `Sanctions Procedure`, `INC-2001…2005`), and the portal `WIRE_REVIEWER` release flow (`GET /wire/queue` · `GET /wire/exceptions` · role-gated `POST /wire/release`; no `release_wire` agent tool). **The only thing left project-wide is the end-of-project `workflow_dispatch` eval sweep + refreshed `evals/SCORECARD.md`** (owner decision). New work starts from a fresh product ask.
