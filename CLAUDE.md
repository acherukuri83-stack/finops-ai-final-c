...
## Current phase: E — Developer Agent (in progress)

Phases A + C + D are complete (D: `platform_api/events/` outbox bus + in-process consumer + dedup + urgency; `make emit`). **Phase E is in progress** — the Developer Agent, **incident mode**: a `platform` MCP server (in-process, fixture-backed — simulated job scheduler / deploy pipeline / config service / bus) with reads + three approval-gated writes (`open_change_ticket`, `rerun_job`, `replay_message`). `agent_core/developer.py` `investigate_incident(subject_id)` runs the `developer` specialist (scope `platform` + `ops` + read-only `trade`; **no deploy/merge/approve tool exists**), finds the causing change + blast radius, and `fix_strategy` (revert vs fix_forward) is a **hard rule in code** keyed on the deployment's `release_note`. `POST /diagnose`; portal Engineering tab. Read `ai-platform/mcp_servers/platform/CLAUDE.md`. Deferred to a later E pass (see `docs/backlog.md`): verification / PR-review / eval-authoring modes, the `repo` + `ci` servers, the Supervisor hand-off. Still out of scope: prime-finance. Wires (B) is an optional module. **No eval sweeps for C or later phases** (owner decision). When Phase E's core slice is merged, bump to "E — core slice complete · next mainline phase: F".
