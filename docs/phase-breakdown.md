...
| | |
|---|---|
| **Core slices built (2026-09-09)** | **Incident mode** — `platform` MCP server (in-process, fixture-backed): reads + approval-gated writes (`open_change_ticket` / `rerun_job` / `replay_message`). `agent_core/developer.py::investigate_incident`, `developer` allowlist (no deploy/merge/approve — asserted), `planner/incident.md` + `synthesis/incident.md`, `Finding.blast_radius` + `Finding.fix_strategy` (hard rule in code, keyed on `release_note`). `POST /diagnose`. **Verification mode** — `verify_change(ticket_id)`: `apply_change_ticket` (deterministic effect), re-check job run + topic lag, residual trades → Settlement specialist (`sub_findings`), write a retrievable `INC-3xxx` on a clean fix; a failed fix reports and re-enters incident mode with the hypothesis excluded — **no second autonomous fix**. `POST /verify`, `platform.get_incident` tool, Engineering tab verify input. **Deferred** (`docs/backlog.md`): PR-review / eval-authoring modes, the `repo` + `ci` servers, the Supervisor hand-off, the standards corpus. |
