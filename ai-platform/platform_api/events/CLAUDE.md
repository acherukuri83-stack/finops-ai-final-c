...
- **Routing is code, not an LLM classify.** A FAILED settlement event carries `trade_id`
  + `failure_code`; the consumer calls `investigate(trade_id)` directly. Urgency comes
  from a `deadline` field, not a model call.
...
- Wire / client event subjects are `OUT_OF_SCOPE` here — they arrive with their modules.
